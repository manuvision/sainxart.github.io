...
#include <colorspace_fragment>
 ...
 function house(x,z,accent){const g=new T.Group();g.userData.silhouetteOccluder="house";g.position.set(x,0,z);scene.add(g);const roof=mat(accent),wall=mat('#b7a77c');box(0,.2,0,3.6,.38,2.7,stone,g);box(0,1.3,0,3.3,2.15,2.45,wall,g);
 ...
 function tree(x,z,s=1){const g=new T.Group();g.userData.silhouetteOccluder="broadleaf";g.position.set(x,0,z);g.scale.setScalar(s);scene.add(g);cylinder(0,1.25,0,.18,2.5,bark,.28,g);for(let i=0;i<4;i++){let a=i*1.57;segment(new T.Vector3(0,.45,0),new T.Vector3(Math.cos(a)*.47,.03,Math.sin(a)*.47),.09,bark,g);segment(new T.Vector3(0,1.55,0),new T.Vector3(Math.cos(a)*.58,2.5,Math.sin(a)*.58),.085,bark,g);}
 ...
 scene.updateMatrixWorld(true);const batches=new Map(),remove=[];scene.traverse(o=>{if(!o.isMesh||o.userData.dynamic)return;let owner=o;while(owner&&!owner.userData.silhouetteOccluder)owner=owner.parent;const occluderKind=owner?.userData.silhouetteOccluder||null;const key=o.material.uuid+o.castShadow+occluderKind;let batch=batches.get(key);if(!batch){batch={material:o.material,cast:o.castShadow,occluderKind,positions:[],normals:[],uvs:[]};batches.set(key,batch);}const g=(o.geometry.index?o.geometry.toNonIndexed():o.geometry.clone()).applyMatrix4(o.matrixWorld);batch.positions.push(...g.attributes.position.array);batch.normals.push(...g.attributes.normal.array);if(g.attributes.uv)batch.uvs.push(...g.attributes.uv.array);else for(let i=0;i<g.attributes.position.count;i++)batch.uvs.push(0,0);g.dispose();remove.push(o);});for(const o of remove)o.removeFromParent();for(const b of batches.values()){const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(b.positions,3));g.setAttribute('normal',new T.Float32BufferAttribute(b.normals,3));g.setAttribute('uv',new T.Float32BufferAttribute(b.uvs,2));const o=new T.Mesh(g,b.material);o.castShadow=b.cast;o.receiveShadow=true;o.userData.silhouetteOccluder=b.occluderKind;if(b.occluderKind)o.layers.enable(1);scene.add(o);}
 ...
 return {colliders,interactables,blocked,groundHeight,cut,update,snapshot:()=>({silhouetteOccluders:[...new Set([...batches.values()].map(b=>b.occluderKind).filter(Boolean))],cut:cutCount,total:herbs.length,burst:debris.length,herbs:herbs.map(h=>({id:h.id,x:h.x,z:h.z,cut:h.cut})),bridge:{x:[4.92,8.92],z:[-.18,1.18]}})};
}
