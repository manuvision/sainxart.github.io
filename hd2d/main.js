...
document.addEventListener('keydown',e=>{if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight',' ','KeyW','KeyA','KeyS','KeyD'].includes(e.code))e.preventDefault();if(e.repeat)return;keys.add(e.code);if(['KeyE','Space','Enter'].includes(e.code))action('interact');if(['KeyB','KeyX'].includes(e.code))action('attack');if(e.code==='Escape'){dismiss();$('#help-panel').hidden=true;}});
...
$('#dismiss').onclick=dismiss;$('#prompt').onclick=()=>action('interact');$('#interact').onclick=()=>action('interact');$('#attack').onclick=()=>action('attack');$('#help').onclick=()=>{$('#help-panel').hidden=!$('#help-panel').hidden;keys.clear();};$('#close-help').onclick=()=>$('#help-panel').hidden=true;
...
stick.onpointerdown=e=>{stickPointer=e.pointerId;stick.setPointerCapture(e.pointerId);stickMove(e);};stick.onpointermove=e=>{if(e.pointerId===stickPointer)stickMove(e)};function stickEnd(){stickPointer=null;joystick={x:0,z:0};$('#thumb').style.transform='';}stick.onpointerup=stick.onpointercancel=stick.onlostpointercapture=stickEnd;
