...
const TOWN={name:'Pallet Town',source:[8,24,384,320],bounds:[32,33,352,307],solids:[
 [80,56,80,72],[224,56,80,72],[208,152,112,72], // houses and laboratory
 [66,106,12,22],[210,106,12,22], // mailboxes
 [80,176,80,16],[80,192,64,32],[80,224,16,16], // fenced garden
 [208,256,96,16],[112,272,64,48] // fence and water
]};
const ROOMS={
 home:{name:'The little red house',source:[400,24,208,160],bounds:[16,40,208,144],solids:[[16,0,96,40],[80,64,64,32],[160,24,48,32],[16,96,16,32],[192,96,16,32]],spawn:[72,141],exit:[72,157],outside:[104,141]},
 rival:{name:'The house next door',source:[400,208,208,160],bounds:[0,40,208,144],solids:[[0,0,96,40],[80,64,64,32],[176,16,32,32],[0,112,16,32],[192,112,16,32]],spawn:[72,141],exit:[72,157],outside:[248,141]},
 lab:{name:"Professor Oak's lab",source:[816,24,208,224],bounds:[0,32,208,208],solids:[[0,0,208,40],[16,64,32,32],[0,48,16,32],[128,64,48,32],[0,120,80,32],[128,120,80,32],[0,176,16,32],[192,176,16,32]],spawn:[104,205],exit:[104,221],outside:[264,237]}
};
const DOORS=[{x:104,y:125,id:'home'},{x:248,y:125,id:'rival'},{x:264,y:221,id:'lab'}];
const SIGNS=[
 {x:152,y:184,title:'PALLET TOWN',text:'A quiet beginning. Every great adventure starts with a few small footsteps.'},
 {x:88,y:232,title:'THE FLOWER GARDEN',text:'Please admire the flowers. Tiny paws belong on the path.'},
 {x:264,y:264,title:"OAK'S LABORATORY",text:'Curiosity is welcome here. The door is just north of this sign.'}
];
...
export class PocketGame{
 constructor(canvas,onChange){
  ...
  this.onChange=onChange;this.player={x:184,y:157,dir:'south'};this.room='town';this.held=new Map();this.walking=false;this.walkTime=0;this.step=null;
 ...
 async load(){
  ...
  [this.mapImage,this.sprites,this.atlas]=await Promise.all([loadImage(asset('pallet-town-sheet.png')),loadImage(asset('raccoon-master.png')),fetch(asset('raccoon-master.json')).then(r=>{if(!r.ok)throw new Error('Missing sprite metadata');return r.json();})]);
 ...
 interact(){
  ...
  if(this.room!=='town'){
   ...
   this.message(this.room==='lab'?'A NOTE ON THE DESK':'A MOMENT OF QUIET',this.room==='lab'?'The world is full of things we have yet to discover. Keep your eyes open, little traveler.':'Sunlight, wooden floors, and the smell of home. You could get used to this town.');return;
  }
  ...
  if(this.player.y>248&&this.player.x>98&&this.player.x<190){this.message('AT THE WATER’S EDGE','The sea can wait. Today, there is a whole little town to get to know.');return;}
  this.message('A LITTLE ADVENTURE','Follow the paths. Read a sign. Step inside the three houses. There is always something to find.');
 }
